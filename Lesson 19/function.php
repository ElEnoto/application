...
function downloadPhoto()
{
    if (!empty($_FILES['photo'])) {
        ...
        if ($_FILES['photo']['name'] == '') {
            echo 'Не удалось загрузить фотографию. Файл не выбран!' . '<br>';
            return show();
        }
        if ($_FILES['photo']['size'] >= MAX_FILE_SIZE) {
            echo "Не удалось загрузить фотографию. Файл не должен превышать размер в 5 Мб!" . '<br>';
            return show();
        }
        if (file_exists('data_copies.json') and file_exists('data_files.json')) {
            $content_files = json_decode(file_get_contents('data_files.json'), true);
        ...
        Файл должен иметь одно из известных расширений графических изображений (gif, jpeg или png)!" . '<br>';
            return show();
        }
